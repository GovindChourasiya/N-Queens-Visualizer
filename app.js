...
const queen = '<i class="fas fa-chess-queen" style="color:#FFDE22"></i>';
...
class Queen {
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#3d472c")
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "#bdc4b4");
        }
    }
